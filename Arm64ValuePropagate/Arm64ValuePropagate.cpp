...
namespace {
struct Arm64ValuePropagatePass : public ModulePass {
  static char ID;
  Arm64ValuePropagatePass() : ModulePass(ID) {}

  virtual bool runOnModule(Module &M) override;
  void valuePropagate(Instruction *Inst, PropaChain &cur_propa_chain, int depth,
                      Function *F);
...
bool Arm64ValuePropagatePass::runOnModule(Module &M) {
...
void Arm64ValuePropagatePass::valuePropagate(Instruction *Inst,
                                             PropaChain &cur_propa_chain,
                                             int depth, Function *F) {
    ...
    for (User *U : Inst->users()) {
      if (Instruction *U_inst = dyn_cast<Instruction>(U)) {
        ...
        if (!inst_exist) {
          ...
          // isRetInst
          if (U_inst->getOpcode() == 1) {
            // return type vector<pair<Inst*, Function*>>
            // get call callsite of current function
            ...
            for (auto CallSite : cur_call_sites) {
              ...
              valuePropagate(CallSite.first, cur_propa_chain, depth + 2,
                             CallSite.second);
            }
            // isStore type
          } else if (U_inst->getOpcode() == 33) {
            ...
            for (User *U : store_inst->getPointerOperand()->users()) {
              ...
              // store is self's user
              // outs() << store_user_inst->hasName() << "\n";
              if (store_user_inst != U_inst) {
                // outs() << store_user_inst->getOpcode() << "\n";
                for (int i = 0; i < depth + 1; i++)
                  outs() << "  ";
...
CallSites Arm64ValuePropagatePass::getCallSites(Function *called_function) {
  CallSites cur_call_sites;
...
bool Arm64ValuePropagatePass::rePropagateDetech(Instruction *U_inst,
                                                PropaChain &cur_propa_chain) {
...
void Arm64ValuePropagatePass::printSourceInfo(Instruction *Inst) {
...
char Arm64ValuePropagatePass::ID = 0;

static RegisterPass<Arm64ValuePropagatePass> X("Arm64ValuePropagate",
...
- value propagate via call: value as a function parameter
 - Example: ###8 cmos_suspend:   call void @rtc_update_irq(%struct.rtc_device*
%10, i64 1, i64 %conv8.i) #9, !dbg !8561
- implicit control flow: value as a br target
 - Example: ###7 cmos_suspend:   br i1 %or.cond49, label %if.then20, label
%if.end27, !dbg !8626
